...
import com.hong.mylivepusher.util.TextureUtil;

import java.nio.ByteBuffer;
import java.nio.ByteOrder;
import java.nio.FloatBuffer;

public class HImgVideoRender implements HEGLSurfaceView.HGLRender{
    ...
    private HImgFboRender hImgFboRender;

    private int srcImg = 0;

    HImgVideoRender(Context context) {
        this.context = context;

        hImgFboRender = new HImgFboRender(context);
    ...
    public void onSurfaceChanged(int width, int height) {
        ...
        hImgFboRender.onChange(width, height);

    }

    @Override
    public void onDrawFrame() {
        imgTextureId = TextureUtil.loadTexture(srcImg, context);
        Log.d("Hong", "id is : " + imgTextureId);
        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, fboId);
        ...
        GLES20.glDeleteTextures(1, ids, 0);//必须删除，否则会内存泄漏

        GLES20.glBindFramebuffer(GLES20.GL_FRAMEBUFFER, 0);
        hImgFboRender.onDraw(textureId);
